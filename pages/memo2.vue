...
export default {
    ...
    computed: {
        memo: function(){ return this.$store.state.memo2.memo; },
        page_items: function(){
            if (this.find_flg){
                var arr = [];
                var data = this.$store.state.memo2.memo;
                ...
                return this.$store.state.memo2.memo.slice(
                    this.num_per_page * this.$store.state.memo2.page, 
                    this.num_per_page * (this.$store.state.memo2.page + 1));
            }
        },
        page: {
            get: function(){
                return this.$store.state.memo2.page;
            },
            set: function(p) {
                var pg = p > (this.$store.state.memo.memo2.length - 1)
                    / this.num_per_page ? 
                    Math.ceil((this.$store.state.memo.memo2.length - 1)
                    / this.num_per_page) - 1 : p;
                pg = pg < 0 ? 0 : pg;
                this.$store.commit('memo2/set_page', pg);
            }
        },
    },
    methods: {
        ...
        insert: function(){
            this.$store.commit('memo2/insert', 
        ...
        remove: function(){
            if (this.sel_flg == false){
                ...
                this.$store.commit('memo2/remove', this.sel_flg);
                this.set_flg();
            }
        },
        find: function(){
    ...
    created: function(){
        this.$store.commit('memo2/set_page',0);
    },
}
</script>
